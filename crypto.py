...
class Crypto(BaseModel):
    ...
    klines_1h: List[List[float]] = Field(
        default_factory=list
    )  # Add klines_1h attribute
    ...
    @computed_field
    @property
    def volatility_factor_1h(self) -> float:
        klines_1h = self.klines_1h
        high = max(kline[2] for kline in klines_1h)
        low = min(kline[3] for kline in klines_1h)
        return ((high - low) / low) * 100 if low != 0 else 0
    ...
    @computed_field
    @property
    def is_uptrend_1h(self) -> bool:
        return TechnicalAnalysis.is_uptrend_1h(
            self.klines_1h
    ...
    def calculate_ma_rsi(self) -> Dict[str, float]:
        ...
        return {
            ...
            "ma_1h": float(
                TechnicalAnalysis.calculate_ma(self.klines_1h, ma_period).iloc[-1]
            ),
            "rsi_1m": float(
                TechnicalAnalysis.calculate_rsi(self.klines_1m, rsi_period).iloc[-1]
            ),
            "rsi_1h": float(
                TechnicalAnalysis.calculate_rsi(self.klines_1h, rsi_period).iloc[-1]
            ),
        }
